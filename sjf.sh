...
sort_by_sjf() {
  ...
  local next_at shortest_idx shortest_bt min_arrived
  ...
  for (( i=0; i<n; i++ )); do
    min_arrived=0
    for (( j=0; j<n; j++ )); do
      if (( completed[j] == 0 && arrival_time[j] <= current_time )); then
        min_arrived=1
      fi
    done

    if (( min_arrived == 0 )); then
      next_at=${arrival_time[0]}
      for (( j=0; j<n; j++ )); do
        if (( completed[j] == 0 && arrival_time[j] < next_at )); then
          next_at=${arrival_time[j]}
        fi
      done
    ...
    for (( j=0; j<n; j++ )); do
      if (( completed[j] == 0 && arrival_time[j] <= current_time )); then
        if (( burst_time[j] < shortest_bt )); then
          shortest_bt=${burst_time[j]}
          shortest_idx=$j
        fi
      fi
    done
    ...
    current_time=$((current_time + burst_time[shortest_idx]))
    order+=($shortest_idx)
  done

  local -a new_p=() new_at=() new_bt=()
  for idx in "${order[@]}"; do
    new_p+=("${processes[$idx]}")
    new_at+=("${arrival_time[$idx]}")
    new_bt+=("${burst_time[$idx]}")
  done
